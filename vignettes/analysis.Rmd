...
**This file contains the code for reproducing the results shown in the related manuscript (numbers, tables, figures). We obtained our results on 2023-08-31 using R version 4.3.0 (2023-04-21) running natively on a MacBook Pro (2020) with an Apple M1 chip, 16 GB memory, and the operating system macOS Ventura (version 13.5.1). The total computation time was around 7.5 hours.**
...
remotes::install_github("LCSB-BDS/transreg") # ref="7057abc0", version 0.99.2
rm(pkgs)
```
    ...
    for(iter in seq_len(nrow(frame))){
      ...
      if(mode=="ext"){ 
        ...
        data <- transreg:::simulate(p=2000,n.target=n.target,family=frame$family[iter],
                                ...
                                trans=c(FALSE,TRUE,TRUE)) # Decide between p=2000 (nicer) and p=500 (faster). 
        target <- data$target
        source <- data$source
        beta <- data$beta
      }
      ...
      if(FALSE){
        # debug xrnet
        test <- xrnet::tune_xrnet(x=target$x[foldid.ext==0,],y=target$y[foldid.ext==0],external=source,family=frame$family[iter],penalty_main=xrnet::define_penalty(penalty_type=frame$alpha[iter]))
      }
      frame[iter,names(loss$deviance)] <- loss$deviance
    }
    save(frame,file=paste0(dir,"/results/sim_",mode,"_fast.RData"))
}
...
<<init>>
for(mode in c("ext","int")){
    file <- paste0(dir,"/results/sim_",mode,"_fast.RData")
