...
for(mode in c("ext","int")){
    ...
    all <- mean <- sd <- pvalue <- matrix(NA,nrow=max(number),ncol=length(loss.var),dimnames=list(1:max(number),loss.var))
    for(i in 1:max(number)){
      for(j in seq_along(loss.var)){
        cond <- number==i
        x <- loss$glmnet[cond]
        y <- loss[[loss.var[j]]][cond]
        pvalue[i,j] <- stats::wilcox.test(x=x,y=y,paired=TRUE,alternative="greater",exact=FALSE)$p.value
        mean[i,j] <- mean(y)
        sd[i,j] <- sd(y)
    ...
    all[] <- paste0(format(round(mean,digits=1))," $\\pm$",format(round(sd,digits=1)))
    grey <- mean>=mean[,"glmnet"]
    all[grey] <- paste0("\\textcolor{gray}{",all[grey],"}")
    min <- cbind(seq_len(max(number)),apply(mean,1,which.min))
    all[min] <- paste0("\\underline{",all[min],"}")
    all <- gsub(pattern=" ",replacement="\\\\phantom{1}",x=all) # trial
    star <- pvalue<=0.05
    all[star] <- paste0(all[star],"*")
    all[!star] <- paste0(all[!star],"\\phantom{*}")
    
   #  loss <- all
   #  quote <- matrix(NA,nrow=nrow(loss),ncol=ncol(loss))
   #  quote[1,] <- FALSE
   #  quote[2,] <- loss[1,]==loss[2,] & loss[1,]==loss[3,] & loss[1,]==loss[4,]
   # quote[3,] <- loss[2,]==loss[3,] & loss[3,]==loss[4,] & loss[4,]==loss[5,]
   #   for(i in 4:(nrow(loss)-2)){
   #     quote[i,] <- (loss[i,]==loss[i-1,] & loss[i,]==loss[i-2,] & loss[i,]==loss[i-3,]) | 
   #     (loss[i,]==loss[i-1,] & loss[i,]==loss[i+1,] & loss[i,]==loss[i+2,])
   # }
   # quote[nrow(quote),] <- loss[nrow(quote),]==loss[nrow(quote)-1,] &
   ...
   #         loss[nrow(quote),]==loss[nrow(quote)-3,]
    ...
    colnames(info) <- sapply(colnames(info),function(x) switch(x,"Ka"="$K_a$","K"="$K$","h"="$h$","alpha"="$\\alpha$","rho.x"="$\\rho_x$","rho.beta"="$\\rho_\\beta$","cor.x"="$\\bar{\\rho}_{x}$","cor.beta"="$\\max(\\hat{\\rho}_{\\beta})$","cor.t"="$\\bar{\\rho}_{a}$",x))
    colnames(all) <- paste0("\\texttt{",colnames(all),"}")
    
    align <- paste0("|r|",paste0(rep("r",times=ncol(info)),collapse=""),"|",paste0(rep("r",times=ncol(all)),collapse=""),"|")
    xtable <- xtable::xtable(x=cbind(info,all),align=align,caption=caption)
    xtable::print.xtable(x=xtable,
                include.rownames=FALSE,
                floating=TRUE,
                sanitize.text.function=identity,
