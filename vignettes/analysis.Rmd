...
for(i in seq_len(nrow(frame))){
  ...
  loss[[i]] <- transreg:::compare(target=list(y=y,x=X[,cond]),prior=prior[cond],family="binomial",alpha=frame$alpha[i],scale=c("exp","iso"),sign=FALSE,switch=FALSE,select=FALSE,foldid.ext=foldid$foldid.ext,foldid.int=foldid$foldid.int,type.measure=c("deviance","class","auc")) # was without foldid.int
...
The following chunk generates the figure for the internal application. **Requires execution of previous chunk.**

- input: results/app_int.RData

- execution time: some seconds

- output: manuscript/figure_int.pdf
...
```{r trial_stability,eval=FALSE,echo=FALSE}
